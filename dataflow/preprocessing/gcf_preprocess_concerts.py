...
def run(options):
    ...
    with beam.Pipeline(options=options) as pipeline:
        ...
        # Load configs
        metadata = io.load_yaml(filename='configs/metadata_formatted_concerts.yml')

        # Specify query
        hist_trans_query = io.load_sql_query('sql/pull_training_data_formatted_concerts.sql').format(
